...
function BookInfoOverlay({ book }: { book: Book | null }) {
  ...
  return (
    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-background-800 border border-background-700 rounded-lg p-6 shadow-xl w-[28rem] z-50 pointer-events-none min-h-[10rem]">
      <div className="absolute -top-3 -right-3">
        {book.currentlyReading && (
          <span className="text-xs font-semibold px-2 py-0.5 bg-accent-400 text-background-900 rounded-full whitespace-nowrap">
            In Progress
          </span>
        )}
      </div>
